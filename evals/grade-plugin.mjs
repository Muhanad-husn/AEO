...
// than fixed by a role. Phase 6 adds a skill and no agent again: new-project is the
// scaffolder, and it triggers on description rather than being a lane, because the founder
// asking for a new project does not know its name. These are the current inventory, not the
// Phase 0 inventory, and the expectation text below says where each part of the number comes
// from so the citation stays honest as the plugin grows.
...
function checkInventory(pluginRoot, results) {
  ...
  add(
    results,
    `the plugin ships exactly ${EXPECTED_SKILL_COUNT} skills (docs/PLAN.md Phase 0's eleven, plus Phase 3's monitor-design, Phase 4's verify, Phase 5's worker-dispatch and Phase 6's new-project)`,
