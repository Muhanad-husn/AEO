// The harness's own cost (#185, moved here from scripts/score/harness.mjs by slice
// 05-harness-cost): node processes fired per tool call, lines a session reads before
// its first action, and lines of tests over lines of source for the consumer as
// configured on this machine.
//
// `measureCost` is `measure` split into the two numbers a session-start section can
// afford (`processes`, `sessionStartLines`) without paying for `testsOverSource`,
// which walks the whole consumer tree (or shells out to `git ls-files`). `measure`
// itself is unchanged in shape: it adds `tests` on top of `measureCost`'s two fields,
// and `scripts/score/harness.mjs` re-exports it as-is so
// `tests/scripts/score-harness.test.mjs` is untouched by the move.
//
// `measureCost` and `measure` are pure node:fs over the two roots they are given
// (`homeDir` and `pluginRoot`). Naming note: `homeDir` here is the directory that
// holds `settings.json`, `CLAUDE.md` and (by default) the `plugins` folder — on a
// real machine that is `<os.homedir()>/.claude`, not `os.homedir()` itself. The
// default below makes that concrete so a fixture can pass a stand-in directory with
// the same flat shape (settings.json, CLAUDE.md, plugins/) without needing a nested
// `.claude` folder of its own.
...
// Pure filesystem read of the harness a consumer runs on this machine: node
// processes fired per tool call and lines read before the first action.
// `homeDir` defaults to `<os.homedir()>/.claude` and `pluginRoot` to
// `<homeDir>/plugins` — see the module comment for why `homeDir` is that
// directory rather than the OS home directory itself. Cheap enough for a
// session-start section to call on every render.
export function measureCost(dir, { homeDir = join(homedir(), '.claude'), pluginRoot = join(homeDir, 'plugins') } = {}) {
...
// The full measurement: `measureCost` plus tests-over-source, which walks the whole
// consumer tree (or shells out to `git ls-files`) and is worth paying for only when a
// caller actually reports the ratio (scripts/score.mjs does; the sensorium's
// session-start section does not).
export function measure(dir, options = {}) {
  return { ...measureCost(dir, options), tests: testsOverSource(dir) };
}
