...
Run the tests this slice writes or touches. If the change reaches a module with acceptance-level contracts beyond that scope, run those too before asking for approval, or wait for CI to go green first. The commit gate's own tier is narrow by design; this is the cost of that, not a gap to route around.
