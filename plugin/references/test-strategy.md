...
- **`test_full` is optional and falls back to `test`.** A project with one honest run, a small suite or a walking skeleton, names one key and is right to. Add the second key when the tiers genuinely diverge, not before.
...
Chasing the symptom N times is what this section exists to stop, and it is where a day goes. Test plumbing that costs more to debug than the behaviour it covers trips the over-engineering rule about a fix larger than its bug, and that tripwire applies to test code too.
