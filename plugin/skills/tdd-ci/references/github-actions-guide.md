...
> Action major versions (`actions/checkout@v4`, `actions/setup-node@v4`, `actions/upload-artifact@v4`, …) and runner images move over time. Verify the current versions by checking the GitHub Actions repository releases or the GitHub Actions docs before committing.
