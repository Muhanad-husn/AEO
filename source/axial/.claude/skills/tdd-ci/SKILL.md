...
- The sprint suite (src units + this sprint's acceptance directory) passes **locally** first. Do not promote red or unrun tests to CI. The full `tests/` tree runs in CI, never locally.
...
Once the workflow is committed and valid, recommend `safe-pr` to open the pull request — it will push the branch (triggering this workflow) and assemble the evidence-rich PR. If running under `sprint-start`, return to the orchestrator.
