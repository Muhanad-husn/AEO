...
5. **Write the plans.** Create `plans/<feature-slug>/`:
   ...
   - Fill in every section. The crucial ones: the **acceptance criterion** written as a Given/When/Then that becomes the slice's failing acceptance test (the builder writes it first and watches it fail), and the **initial unit test list** that seeds the inner loop (inner unit tests live co-located under `src/`, not in `tests/`). These don't have to be exhaustive — the test list is living — but they must pin down "done".

6. **Confirm and hand off.** Show the founder the slice list (titles + one-line goals + the proposed first slice). Get sign-off before any code is written. Then the pipeline continues per `sprint-start`: the builder takes slice `01` through `red-green-refactor` to a PR.

## What makes a plan good

- **One behaviour per slice.** If you can't state the slice's value in a single sentence, it's too big.
- **The simplest mechanism that meets the bar.** The plan names the mechanism and keeps it minimal; generality, tunables, and polish are deferred to the out-of-scope section by default. If the mechanism needs hand-tuned constants over messy real-world data, consider whether a model call is the simpler machine.
