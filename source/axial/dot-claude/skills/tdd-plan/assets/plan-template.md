...
- **Outer test file (planned):** <path under tests/<subproject>/, e.g. tests/<subproject>/test_<feature>_<slice>.py — written first by the builder, watched failing, committed together with the code that greens it>

## Inner loop — initial unit test list

Seed for the builder's inner red-green-refactor cycles (unit tests live
co-located under `src/`, e.g. `src/**/test_*.py` — never in `tests/`, which holds
the behavioral contracts). This is a **living list** — `red-green-refactor`
...
- [ ] Acceptance test written first and seen to fail for the right reason.
- [ ] All seeded unit behaviours covered; sprint suite passes locally; acceptance test GREEN.
- [ ] Refactor pass complete (no duplication, clear names) with the bar green.
- [ ] Slice's tests run in CI (`tdd-ci`).
- [ ] Review run if warranted (founder-requested or high-blast-radius); findings addressed or logged.
