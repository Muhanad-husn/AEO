...
6. **Retire the worktree with the branch.** Every issue and fix is built in
   `.claude/worktrees/<slug>`, so a deleted branch leaves a full second checkout
   on disk. For each branch just deleted, run `git worktree remove <path>` (add
   `--force` only if the founder confirms the leftover changes are disposable),
   then `git worktree prune`. Never touch `.claude/worktrees/axial-harness` — it
   is the harness snapshot mirror, not slice work.

7. **Optional bookkeeping.** If a deleted branch's slice plan still says in-progress, update `plans/<feature>/<NN>-<slice>.md` / the feature README to reflect that the slice is merged and its branch cleaned.
